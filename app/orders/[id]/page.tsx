  ...
  if (!id) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-black py-10 px-4">
        <div className="bg-white p-10 rounded-3xl shadow-xl text-center border border-red-100 dark:bg-gray-900">
          <div className="text-6xl mb-4">❌</div>
          <h1 className="text-3xl font-bold text-red-600 dark:text-white"> Invalid Order ID </h1>
          <Link href="/orders">
            <button className="mt-6 px-6 py-3 bg-black dark:bg-white text-white dark:text-black rounded-xl font-semibold"> Back to Orders </button>
          </Link>
        </div>
      </div>
    );
  ...
  if (!order) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-white px-4">
        <div className="bg-white p-10 rounded-3xl shadow-xl text-center border border-red-100">
          <div className="text-6xl mb-4">😢</div>
          <h1 className="text-3xl font-bold text-gray-800"> Order not found </h1>
          <Link href="/orders">
            <button className="mt-6 px-6 py-3 bg-red-600 text-white rounded-xl font-semibold"> Back to Orders </button>
          </Link>
        </div>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white dark:bg-[#071225] rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 mb-6">
          ...
          <div className="text-right">
            ...
            <p className="text-3xl font-bold text-green-700"> ₹{order.amount} </p>
          </div>
        </div>
        ...
        <div className="space-y-5">
          {order.items.map((item) => (
            <div
              key={item.id}
              className="bg-white dark:bg-[#071225] rounded-3xl p-5 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-2xl transition"
            >
              <div className="flex gap-5 items-center">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-4 mt-3 text-sm">
                    ...
                    <span className="bg-green-100 dark:bg-gray-700 px-3 py-1 rounded-full text-green-700 dark:text-green-300 font-semibold"> ₹{item.priceAtPurchase} each </span>
                  </div>
                </div>
